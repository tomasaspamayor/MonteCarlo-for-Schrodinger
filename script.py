...
for k in range(P):
    pdf_qo = partial(pdfs.wf_pdf, n=k, coeffs=hermite_coeffs)
    samples_wf_r = samp.rejection(pdf_qo, 0, -4.6, 4.5, 500000, 5000000, m=None)
    samp.plot_samples(pdf_qo, [-4.6, 4.5], samples_wf_r, 200, 0)

    local_energies_r = le.analytical_local_energy(samples_wf_r, hermite_coeffs, k)
    energy_k_r = np.sum(local_energies_r) / len(local_energies_r)
    print(f'The local energy for the wavefunction of order {k} is {energy_k_r} (Rejection Method Sampling)')

for k in range(P):
    pdf_qo = partial(pdfs.wf_pdf, n=k, coeffs=hermite_coeffs)
    samples_wf_mh = samp.metropolis_hastings(pdf_qo, 0, [-4.5, 4.5], 0.1, 500000, 250000)
    samp.plot_samples(pdf_qo, [-4.5, 4.5], samples_wf_mh, 200, 1)

    local_energies_mh = le.analytical_local_energy(samples_wf_mh, hermite_coeffs, k)
